const STORAGE_KEY = "comandaAzulState_v9_outros_dicas_reais";
...
const products = [
  ...
  { name: "Outros", price: 0, custom: true }
];
...
const rotatingTip = document.querySelector("#rotatingTip");
const tipCounter = document.querySelector("#tipCounter");

const tips = [
  "use a busca dentro de cada comanda para achar produtos rapidamente. O total da comanda aparece apenas dentro dela.",
  "use Valor avulso quando esquecer de anotar uma comanda; ela entra no fechamento como Comanda e Itens indefinidos.",
  "os valores do Valor do Dia ficam ocultos na tela e aparecem apenas no PDF final.",
  "para conferir o total no topo, use Desocultar com senha e depois oculte novamente.",
  "ao enviar uma comanda, escolha a forma de pagamento para garantir que ela já foi paga.",
  "se o pagamento for em Dinheiro, informe o valor recebido para o sistema calcular o troco.",
  "use Editar no Valor do Dia para corrigir comanda, itens ou forma de pagamento antes de concluir.",
  "adicione Extras quando precisar de mais comandas e remova os Extras que não forem mais usados.",
  "use Outros para lançar um produto fora do catálogo digitando o valor manualmente.",
  "ao concluir o dia, o PDF é baixado automaticamente e o sistema limpa as comandas e remove os Extras."
];
let currentTipIndex = 0;

function updateTipText() {
  if (!rotatingTip) return;
  rotatingTip.textContent = tips[currentTipIndex];
  if (tipCounter) tipCounter.textContent = `${currentTipIndex + 1}/${tips.length}`;
}

function startTipRotation() {
  if (!rotatingTip || tips.length === 0) return;
...
function createItems(oldItems = []) {
  return products.map((product) => {
    ...
    const isCustom = Boolean(product.custom);

    return {
      name: product.name,
      price: product.price,
      qty: isCustom ? 0 : Math.max(0, Math.floor(Number(oldItem?.qty) || 0)),
      custom: isCustom,
      customValue: isCustom ? Math.max(0, Number(oldItem?.customValue ?? oldItem?.value) || 0) : 0
    };
  });
}
...
function cloneSelectedItems(items = []) {
  return createItems(items)
    .filter((item) => item.custom ? item.customValue > 0 : item.qty > 0)
    .map((item) => item.custom
      ? { name: item.name, price: 0, qty: 0, custom: true, customValue: item.customValue }
...
function normalizeDailyEntry(entry) {
  const items = createItems(entry?.items).filter((item) => item.custom ? Number(item.customValue) > 0 : item.qty > 0);
...
function itemsTotal(items = []) {
  return items.reduce((total, item) => {
    if (item.custom) return total + Math.max(0, Number(item.customValue) || 0);
...
function itemsQty(items = []) {
  return items.reduce((total, item) => {
    if (item.custom) return total + (Number(item.customValue) > 0 ? 1 : 0);
...
function itemsText(items = []) {
  return items
    .filter((item) => item.custom ? Number(item.customValue) > 0 : item.qty > 0)
    .map((item) => item.custom ? `${item.name}: ${money(item.customValue)}` : `${item.qty}x ${item.name}`)
    .join(", ");
}
...
function putCustomItemsFirst(items) {
  return [...items].sort((a, b) => Number(Boolean(b.custom)) - Number(Boolean(a.custom)));
}
...
function filteredItems(items, searchTerm) {
  ...
  const list = !term
    ? items
    : items.filter((item) => normalizeForSearch(item.name).includes(term));

  return putCustomItemsFirst(list);
}
...
function renderCommandProducts(command) {
  ...
  return list.map((item) => {
    if (item.custom) {
      return `
        <div class="product-row other-product-row">
          <div class="product-info">
            <strong>${escapeHTML(item.name)}</strong>
            <span>Valor livre / fora do catálogo</span>
          </div>
          <div class="other-value-control">
            <label>R$</label>
            <input type="text" inputmode="decimal" placeholder="0,00" value="${escapeHTML(customValueText(item.customValue))}" data-action="other-value" data-command-id="${command.id}" data-product="${escapeHTML(item.name)}" autocomplete="off" />
          </div>
        </div>
      `;
    }
    ...
    <tr>
      ...
      <td>
        <div class="daily-row-actions">
          <button class="btn btn-soft btn-small" type="button" data-action="edit-daily" data-entry-id="${entry.id}">Editar</button>
          <button class="btn btn-danger btn-small" type="button" data-action="delete-daily" data-entry-id="${entry.id}">Excluir</button>
        </div>
      </td>
    </tr>
  `).join("");
...
function resetCommandItems(command) {
  command.items.forEach((item) => {
    item.qty = 0;
    if (item.custom) item.customValue = 0;
  });
  command.searchTerm = "";
}
...
function updateCustomItemValue(items, productName, value) {
  const item = items.find((entryItem) => entryItem.name === productName && entryItem.custom);
  if (!item) return;
  item.customValue = Math.max(0, parseMoneyInput(value));
}

function customValueText(value) {
  const numericValue = Math.max(0, Number(value) || 0);
  return numericValue > 0 ? String(numericValue.toFixed(2)).replace(".", ",") : "";
}

function updateCommandCardSummary(command) {
  const card = document.querySelector(`[data-command-id="${CSS.escape(command.id)}"]`);
  if (!card) return;

  const total = commandTotal(command);
  const qty = commandQty(command);
  const totalNode = card.querySelector(".command-total strong");
  const qtyNode = card.querySelector(".item-pill");

  if (totalNode) totalNode.textContent = money(total);
  if (qtyNode) qtyNode.textContent = `${qty} ${qty === 1 ? "item" : "itens"}`;
}
...
function renderEditCommandProducts() {
  ...
  editProducts.innerHTML = list.map((item) => {
    if (item.custom) {
      return `
        <div class="edit-product-row other-product-row">
          ...
          <div class="other-value-control">
            <label>R$</label>
            <input type="text" inputmode="decimal" placeholder="0,00" value="${escapeHTML(customValueText(item.customValue))}" data-action="edit-other-value" data-product="${escapeHTML(item.name)}" autocomplete="off" />
          </div>
        </div>
      `;
    }
    ...
    if (item.custom) {
      return `
        <div class="edit-product-row other-product-row">
          ...
          <div class="other-value-control">
            <label>R$</label>
            <input type="text" inputmode="decimal" placeholder="0,00" value="${escapeHTML(customValueText(item.customValue))}" data-action="daily-edit-other-value" data-product="${escapeHTML(item.name)}" autocomplete="off" />
          </div>
        </div>
      `;
    }
...
async function deleteDailyEntry(entryId) {
  ...
  const confirmation = await askConfirmation({
    ...
    message: `Deseja excluir ${entry.origin} da lista do Valor do Dia? Essa ação remove esse lançamento do fechamento.`,
    confirmText: "Excluir",
    cancelText: "Cancelar"
  });
  if (!confirmation) return;

  state.dailyEntries = state.dailyEntries.filter((item) => item.id !== entryId);
  renderAll();
  showToast("Lançamento excluído da lista.");
}
...
commandsGrid.addEventListener("input", (event) => {
  ...
  if (target.dataset.action === "other-value") {
    updateCustomItemValue(command.items, target.dataset.product, target.value);
    updateCommandCardSummary(command);
    saveState();
    return;
  }
...
editProducts.addEventListener("input", (event) => {
  const target = event.target;
  if (target.dataset.action !== "edit-other-value") return;
...
dailyList.addEventListener("click", (event) => {
  ...
  if (target.dataset.action === "delete-daily") {
    deleteDailyEntry(target.dataset.entryId);
  }
});
...
dailyEditProducts.addEventListener("input", (event) => {
  const target = event.target;
  if (!state.dailyEditDraft || target.dataset.action !== "daily-edit-other-value") return;

  updateCustomItemValue(state.dailyEditDraft.items, target.dataset.product, target.value);
  dailyEditItemCounter.textContent = `${itemsQty(state.dailyEditDraft.items)} ${itemsQty(state.dailyEditDraft.items) === 1 ? "item" : "itens"}`;
  updateDailyEditCashPreview();
});
