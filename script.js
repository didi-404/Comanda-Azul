const STORAGE_KEY = "comandaAzulState_v7_seguranca_pdf";
...
function cashDisplay(entry) {
  ...
  return entry.change > 0 ? "Recebido/troco registrados" : "Recebido sem troco";
}
...
function renderDailyEntries() {
  ...
  topDayTotal.textContent = "Oculto";
  dailyTotal.textContent = "Oculto";
  topDayCount.textContent = `${count} ${count === 1 ? "conta enviada" : "contas enviadas"}`;

  if (count === 0) {
    dailyList.innerHTML = `<tr class="empty-row"><td colspan="7">Nenhuma conta enviada ainda.</td></tr>`;
    return;
  }
  ...
  dailyList.innerHTML = state.dailyEntries.map((entry) => `
    <tr>
      ...
      <td><span class="masked-value">Oculto</span></td>
...
function promptRename(command) {
  const newName = normalizeText(window.prompt("Digite o novo nome da comanda:", command.name), 35);
  if (!newName) return;
...
function confirmPaymentAndSend() {
  ...
  const confirmation = window.confirm(`Tem certeza que deseja enviar ${command.name} para o Valor do Dia?`);
...
function clearDayEntries() {
  ...
  if (!window.confirm("Tem certeza que deseja limpar o Valor do Dia? Essa ação não gera PDF.")) return;
  state.dailyEntries = [];
  renderAll();
...
function finishDay() {
  ...
  const confirmation = window.confirm("Concluir o dia irá gerar o PDF, zerar o fechamento, limpar comandas e remover todos os extras. Deseja continuar?");
  if (!confirmation) return;
  ...
  renderAll();
  showToast("PDF gerado. Dia concluído e sistema resetado.");
}
...
function generateDailyPdf() {
  ...
  buildPdf(lines, `relatorio-bar-azul-${filenameDate}.pdf`);
}
...
commandsGrid.addEventListener("click", (event) => {
  ...
  if (action === "clear-command" && command) {
    ...
    if (!window.confirm(`Deseja excluir todos os itens de ${command.name}?`)) return;
    resetCommandItems(command);
    renderAll();
